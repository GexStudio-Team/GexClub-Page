...
- **Imagen "Who We Are"** (`/about`): la sección `&lt;who_we_are&gt;` de la página Sobre Nosotros ahora muestra la fotografía **`Frame2` del video institucional** (`public/brand/frame2.jpg`, 736×981) en lugar del isotipo flotante, con el mismo tratamiento visual (marco sutil `border-primary/20`, glow azul `shadow-[0_0_35px_rgba(22,135,255,0.14)]` y `object-contain` adaptativo `max-h-[80%] max-w-[82%]`) que respeta desktop y móvil.
