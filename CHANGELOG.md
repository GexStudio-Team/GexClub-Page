...
- **Ola-gusano de aurora boreal en el banner de Proyectos** (`src/components/projects/WormWave.jsx`, `/projects`): el recuadro grande que dice **"Proyectos actuales"** (el que tiene junto el isotipo `gex-mark-dark.png`) ahora lleva una banda segmentada tipo **gusano** con **colores pastel vivos de aurora boreal** (rosa `#f9a8d4` → fucsia `#e879f9` → violeta `#c084fc` → azul `#60a5fa` → cian `#22d3ee` → esmeralda `#34d399`) que **cruza el recuadro HORIZONTALMENTE, muy lenta** (16 s), desde la **izquierda hasta la derecha**: entra desde fuera por la izquierda, avanza con cuerpo ondulado de **gusano mezclado con onda** (protuberancias alternas grandes `AMP1 240` / pequeñas `AMP2 130`), con grosor vertical que supera la altura del recuadro para que al pasar **TAPE TODO el banner**, y al final hace un reset rápido y vuelve a empezar (loop 92 % avance + 8 % reset, `calcMode="spline"`). **Glow Path Effect** en el borde superior (halo difuso `feGaussianBlur 30` + brillo nítido `12`), `mix-blend-screen`. El SVG se inyecta **como HTML nativo con `dangerouslySetInnerHTML`** porque React descarta los atributos SMIL (`animateTransform`, `values`, `dur`) al renderizarlos como JSX, lo que impedía que la animación se ejecutara — con HTML string el navegador corre SMIL directamente. La ola va **solo dentro del banner**, no sobre toda la pantalla.
