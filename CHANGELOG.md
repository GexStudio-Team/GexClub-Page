...
- **Aurora boreal SVG Wave + Glow Path** (`src/components/ui/AuroraWave.jsx`): nuevo fondo para el cuadro **"Proyectos actuales"** (`/community`) que reemplaza la aurora CSS simple. Compuesto por **3 capas de ondas SVG** (paths ondulados más anchos que el viewBox) con **`preserveAspectRatio="none"`** — conserva la forma física del cuadro — gradientes de los colores del logo (azul `#008BFE`, violeta `#8b5cf6`/`#a855f7`, cian `#22d3ee`), **Glow Path Effect** (`feGaussianBlur` + `feMerge` en filtros `auroraGlow`/`auroraSoft`) y animaciones **SMIL autocontenidas**: las ondas **pasan horizontalmente por tramos con pausas** (discontinuo) con direcciones y palpitaciones de opacidad desfasadas por capa.
