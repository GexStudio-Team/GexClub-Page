...
- **Ola-gusano en la ventana de Proyectos** (`src/components/projects/WormWave.jsx`, `/projects`): overlay **fijo a pantalla completa** (`fixed inset-0 z-[80] pointer-events-none`) que barre diagonalmente de la **esquina inferior izquierda a la superior derecha** como el movimiento de un **gusano** (no una onda de radio): banda segmentada con protuberancias redondeadas alternadas (`AMP1`/`AMP2`), más ancha que la diagonal del viewport para **tapar toda la pantalla** al pasar, con **cola transparente** (gradiente de opacidad) para que al volver se **destape desde el lugar donde comenzó**, en loop (`calcMode="spline"` + `repeatCount="indefinite"`). Borde delantero con **Glow Path Effect** en colores Gex Club (`feGaussianBlur` + `feMerge`, halo difuso + brillo nítido) y animación SMIL pura.
