...
- **Perfil del fundador — pulido visual** (`/aliados/duvan-altamar`): el header ahora es un grid con el **isotipo compuesto a la derecha** (centrado en móvil) y micro-interacción `hover:scale`; "Cosas que soy" se mejora con **numeración mono (01–06) + coronita** por ítem y separadores `divide-y`; el **placeholder de la foto** gana un ícono de cámara y hover sutil; las **redes pasan a botones full-width** con flecha que se desplaza y glow al hover; se añade un tercer CTA **"Escríbeme"** (`mailto`) junto a GEX_OS y el vault.
