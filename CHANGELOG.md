...
- **Ola-gusano de aurora boreal en el banner de Proyectos** (`src/components/projects/WormWave.jsx`, `/projects`): el recuadro grande que dice **"Proyectos actuales"** (el que tiene junto el isotipo `gex-mark-dark.png`) ahora lleva una banda segmentada tipo **gusano** con **colores pastel vivos de aurora boreal** (rosa `#f9a8d4` → fucsia `#e879f9` → violeta `#c084fc` → azul `#60a5fa` → cian `#22d3ee` → esmeralda `#34d399`) que **cruza el recuadro en diagonal** (esquina inferior izquierda → superior derecha) con `mix-blend-screen`, **Glow Path Effect** (halo difuso `feGaussianBlur 30` + brillo nítido `12`) en el borde delantero, cola transparente que **se destapa desde el lugar donde comenzó** y loop infinito (`calcMode="spline"`, 8 s). El SVG se inyecta **como HTML nativo con `dangerouslySetInnerHTML`** porque React descarta los atributos SMIL (`animateTransform`, `values`, `dur`) al renderizarlos como JSX, lo que impedía que la animación se ejecutara — con HTML string el navegador corre SMIL directamente. La ola va **solo dentro del banner**, no sobre toda la pantalla.
